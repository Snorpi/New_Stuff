    ...
    with open(csv_file, 'r') as csvfile:
        ...
        for row in reader:
            try:
                ...
                course_url = f"""{row['Month']}-{row['Day']}-24-7-{elective_duration}-{row['elective']}-ceq"""
                html_content = f"""
                ...
                <p> </p>
                <p style="text-align: center;"><span style="font-size: 18pt;"><strong>Date: </strong>{row['course_date']}, 2024</span></p>
                ...
                <!-- 2024 7 + {elective_duration} Hour {elective_name} CE Webinar {row['course_date']}, 2024 {row['start_time']} {row['timezone']} -->
                <!-- {row['Month']}.{row['Day']}.2024 - 7 + {elective_duration} {row['elective']} CEQ -->
                <!-- {course_url} -->
                <!-- {row['Month']}/{row['Day']} - 7 + {elective_duration} Hour {elective_name} CE Webinar -->
                <!-- asana info -->
                <!-- {elective_duration}Hr {row['elective']} 
                <h2 style="text-align: center">
                <a
                title="7 + {elective_duration} Hour {elective_name} CE Webinar - {row['course_date']}"
                href="https://mortgageeducators.com/component/virtuemart/{course_url}-detail"
                target="_blank"
                rel="noopener noreferrer"
                >7 + {elective_duration} Hour {elective_name} CE Webinar - {row['course_date']}</a
                >
                </h2>
                """
